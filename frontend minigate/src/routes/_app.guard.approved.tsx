...
export const Route = createFileRoute("/_app/guard/approved")({
  component: Page,
});
...
function VisitorCard({ row, onCheckIn, loading }: { row: any; onCheckIn: () => void; loading: boolean }) {
  const words: string[] = (row.visitor_name || "").trim().split(" ");
  const initials = (words[0]?.[0] ?? "") + (words.length > 1 ? (words[words.length - 1]?.[0] ?? "") : "");
  const canCheckIn = row.status === "approved";

  const statusBg =
    row.status === "inside"   ? "border-success/30 bg-success/10"    :
    row.status === "approved" ? "border-warning/30 bg-warning/10"    :
                                "border-destructive/30 bg-destructive/10";
  const statusText =
    row.status === "inside"   ? "text-success"          :
    row.status === "approved" ? "text-warning-foreground" :
                                "text-destructive";
  const statusLabel =
...
function Page() {
  ...
  const { data: kpiData } = useQuery({
    ...
    queryFn: () => guardService.getApprovedStats().then((r: any) => r.data.data),
  ...
  const checkIn = useMutation({
    mutationFn: ({ source, id }: { source: string; id: number }) =>
      guardService.checkInApproved(source, id),
    onSuccess: (res: any) => {
    ...
    <>
      <PageHeader
        ...
        actions={
          <Button variant="outline" size="sm" className="gap-1.5" asChild>
            <a href="/api/security-guard/approved-visitors/export/" target="_blank">
      ...
      <div className="space-y-6 p-6">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <KpiCard icon={Users}    value={kpi.waiting_at_gate} label="Waiting at Gate" color="text-warning-foreground" bg="bg-warning/10" />
          <KpiCard icon={UserCheck} value={kpi.already_inside}  label="Already Inside"  color="text-success"           bg="bg-success/10" />
          <KpiCard icon={TimerOff}  value={kpi.pass_expired}    label="Pass Expired"    color="text-destructive"       bg="bg-destructive/10" />
        </div>
