...
function ExpenseModal({ expense, societyId, onClose }) {
  ...
  const mutation = useMutation({
    mutationFn: (data) =>
      isEdit ? accountantService.updateExpense?.(expense.id, data) : accountantService.createExpense(data),
    onSuccess: () => {
...
export default function AccountantExpenses() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['accountant-expenses', society?.id, page],
    queryFn: () =>
      accountantService.getExpenses({ society: society?.id, page, page_size: PAGE_SIZE }).then((r) => r.data),
  })

  const publishMutation = useMutation({
    mutationFn: (id) => accountantService.publishExpense(id),
    onSuccess: () => { toast.success('Expense published'); qc.invalidateQueries({ queryKey: ['accountant-expenses'] }) },
    onError: (err) => toast.error(getErrorMessage(err)),
  })
    ...
    { header: 'Date', accessor: 'expense_date', render: (v, row) => formatDate(v || row.created_at) },
    {
      ...
      render: (_, row) => (
        <div className="flex gap-1.5">
          <button onClick={() => setModal({ type: 'edit', expense: row })} className="text-xs text-primary underline underline-offset-2 hover:text-primary/80">Edit</button>
          {row.status === 'draft' && (
            <button
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader
        title="Expenses"
        description="Record and manage society expenses"
        actions={
