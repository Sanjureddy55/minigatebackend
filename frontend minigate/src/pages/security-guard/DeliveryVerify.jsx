...
export default function DeliveryVerify() {
  ...
  const create = useMutation({
    mutationFn: (data) => guardService.createDelivery(data),
    onSuccess: () => {
      toast.success(`Delivery from ${form.agent_name} logged`)
      setForm({ ...EMPTY_FORM })
      qc.invalidateQueries({ queryKey: ['deliveries-today'] })
    },
    onError: (err) => toast.error(err.response?.data?.message ?? 'Failed to log delivery'),
  })

  const approve = useMutation({
    mutationFn: (id) => guardService.approveDelivery(id),
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          pending > 0 ? (
            <span className="inline-flex items-center gap-1.5 rounded-full border border-amber-300/30 bg-amber-100/30 px-3 py-1 text-xs font-bold text-amber-700">
              {pending} pending
            </span>
          ) : undefined
        }
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                ...
                <Select value={form.vendor} onValueChange={(v) => setForm((f) => ({ ...f, vendor: v }))}>
                  ...
                  <SelectContent>{VENDORS.map((v) => <SelectItem key={v.value} value={v.value}>{v.label}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5">
                ...
                <Select value={form.delivery_type} onValueChange={(v) => setForm((f) => ({ ...f, delivery_type: v }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{DELIVERY_TYPES.map((dt) => <SelectItem key={dt.value} value={dt.value}>{dt.label}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5">
        ...
        <div className="rounded-xl border border-border bg-card">
          ...
          <div className="max-h-[560px] overflow-y-auto divide-y divide-border">
            {deliveries.map((d) => {
              ...
              const arrived = d.arrived_at
                ? new Date(d.arrived_at).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })
                : '—'
              return (
                <div key={d.id} className="flex items-start gap-3 px-5 py-3">
                  <div className="h-9 w-9 rounded-xl bg-amber-100/30 text-amber-700 flex items-center justify-center text-xs font-bold flex-shrink-0">
                    {(d.vendor_display || d.vendor || '?').charAt(0).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    {d.status === 'pending' && (
                      <button onClick={() => approve.mutate(d.id)} disabled={approve.isPending}
                        className="mt-1.5 inline-flex items-center gap-1 rounded-full border border-green-300/30 bg-green-100/30 px-2.5 py-0.5 text-[11px] font-semibold text-green-700 hover:bg-green-100/50 transition-colors">
